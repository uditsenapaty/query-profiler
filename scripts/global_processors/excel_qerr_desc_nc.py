# =========================================================
# scripts/excel_qerr_desc_nc.py
#
# Builds ONE beautiful .xlsx for a single gt_results_* folder:
#   - 5 tabs, one per query (qt5, qt7, qt8, qt10, qt16)
#   - each tab shows ALL methods' qerr_desc_nc details, stacked
#     as colored per-method sections
#
...
#   (with the 11x11-points note), per-method sampling kind
#   (uniform / exponential), min-adjustment, and the nc scope.
#
# Output:
#   {gt_root}/summaries_nc/qerr_desc_nc_{run_type}.xlsx
#
# Usage:
#   python scripts/excel_qerr_desc_nc.py                       # default folder
#   python scripts/excel_qerr_desc_nc.py gt_results_sf1_10x10_s1q0
# =========================================================
...
def _global_meta(info, queries):
    ...
    return [
        ...
        ("Methods", methods_legend),
        ("Generated", datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
    ]


def info_folder(info):
    return f"gt_results_sf{info['sf']}_{info['res']}_{info['run_type']}"
...
def _coerce(value):
    """Return (python_value, number_format_or_None)."""
    if value is None:
        return None, None
...
def _meta_block(ws, row, ncols, items):
    for label, value in items:
        ...
        vc.alignment = Alignment(horizontal="left", vertical="center", indent=1, wrap_text=False)
        vc.border = BORDER
...
def run(gt_root):
    ...
    print()
    print("=" * 70)
    print("EXCEL — qerr_desc_nc (per-query tabs, all methods)")
    print("=" * 70)
    ...
    for query in queries:
        methods = discovered[query]
        method_names = sorted(methods)

        # load frames; columns identical across methods of a query
        frames = {}
        for m in method_names:
            frames[m] = pd.read_csv(methods[m])

        cols = list(frames[method_names[0]].columns)
        ...
        row = _title(
            ws, row, ncols,
            f"{query.upper()} — Q-error instances  ·  NON-ZERO CARDINALITY (_nc)",
        )
        row += 1
        per_method_counts = ", ".join(
            f"{m}: {len(frames[m])}" for m in method_names
        )
        meta = _global_meta(info, queries) + [
            ("This tab", f"Query {query.upper()} — rows per method → {per_method_counts}"),
            ("Highlight", "Rows shaded orange = plan change (plan_change = True)"),
        ]
        row = _meta_block(ws, row, ncols, meta)
        row += 1

        for m in method_names:
            df = frames[m]
            color = METHOD_COLORS.get(m, "595959")
            row = _section_header(
                ws, row, ncols,
                f"METHOD {m.upper()}  —  {METHOD_DESC.get(m, m)}   "
                f"[{METHOD_KIND.get(m, '')}]   ·   {len(df)} rows",
                color,
            )
        ...
        _autosize(ws, cols, list(frames.values()))
        ws.freeze_panes = "A1"
        print(f"  + tab {query.upper():5s}  ({', '.join(method_names)})")

    out_dir = gt_root / "summaries_nc"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / (
        f"qerr_details_per_query_sf{info['sf']}_{info['res']}_{info['run_type']}_nc.xlsx"
    )
    wb.save(out_path)
