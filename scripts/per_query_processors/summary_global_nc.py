# =========================================================
# scripts/global_processors/summary_global_nc.py
#
# Collects the per-method summary_nc.csv files of a single query into
# ONE workbook, one tab per method — written inside the query folder.
#
# _nc = non-zero cardinality scope (instances with count_rows = 0 OR
# neighbor_count_rows = 0 removed upstream).
#
# Called by build_gt.py as a global processor (once per query):
#   run(config_gt.RESULTS_DIR.parent)          # a single query dir
#
# Input layout:
#   {query_dir}/m0/summary_nc.csv
#   {query_dir}/m1/summary_nc.csv
#   {query_dir}/m2/summary_nc.csv
#
# Output (one file, tabs = methods, no CSVs):
#   {query_dir}/summary_all_methods_nc.xlsx
# =========================================================
...
SUMMARY_NAME = "summary_nc.csv"                  # per-method input
OUT_NAME     = "summary_all_methods_nc.xlsx"     # per-query output (tabs = methods)
...
def run(results_dir):
    ...
    print()
    print("=" * 60)
    print("SUMMARY GLOBAL (nc) — ONE WORKBOOK, ONE TAB PER METHOD")
    print("=" * 60)
